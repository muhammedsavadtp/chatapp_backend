...
//             return res.status(400).json({ error: 'At least one field (name or profilePicture) is required' });
//         }
...
//             bio: updatedUser.bio,
//             lastSeen: updatedUser.lastSeen ? new Date(updatedUser.lastSeen).toLocaleString() : 'N/A',
//         });
//     } catch (error) {
...
exports.updateProfile = async (req, res) => {
    try {
        ...
        else if (profilePicture === null || profilePicture === 'null' || profilePicture === undefined) {
            updateData.profilePicture = null;
        }

        // Add other fields to updateData only if they exist and are not undefined
        if (name !== undefined) updateData.name = name;
        if (username !== undefined) updateData.username = username;
        if (bio !== undefined) updateData.bio = bio;

        // Check if there's anything to update
        // Allow update if profilePicture is explicitly set to null or if there are other fields
        if (Object.keys(updateData).length === 0 && profilePicture !== null && profilePicture !== 'null') {
