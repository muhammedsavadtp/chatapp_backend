...
exports.addGroupMembers = async (req, res) => {
    ...
    try {
        ...
        // Convert back to ObjectId if needed (Mongoose handles this automatically when saving)
...
exports.deleteGroup = async (req, res) => {
    ...
    try {
        ...
        await Message.deleteMany({ group: groupId }); // Optional: Delete group messages
...
exports.removeGroupMember = async (req, res) => {
    ...
    try {
        ...
        group.admins = group.admins.filter(a => a.toString() !== memberId); // Remove admin privilege if applicable
        await group.save();
        const updatedGroup = await Group.findById(groupId)
            .populate('members', 'username name profilePicture status lastSeen')
            .populate('admins', 'username name');
...
exports.addGroupAdmin = async (req, res) => {
    ...
    try {
        ...
        if (!group.members.includes(memberId)) {
            return res.status(400).json({ error: 'User must be a group member to be an admin' });
        }
